...
Steps performed on each target file:
  ...
  5.  Populate Source_port, DMARC1, DMARC2, Destination_port from Cutsheets
      (matched on Hostname + Interface — no Formula L&R needed)
  5b. Mismatches: add Possible columns (matched via cutsheet Z-side) +
      Active Z columns (from Act. data), both with pink background
  6.  Add Summary tab (first) with row counts
  7.  Remove all fills (except pink Possible/Z cols); apply borders everywhere
  8.  Rename output file to top-2 Rack numbers
"""
...
def process_file(input_path, output_path, cut_df, log):
    ...
    if lldp_tab:
        ...
        # Rename A-side columns to Act. prefix
        ...
        # Rename A-side columns in Mismatches too before writing.
    ...
    desired  = ['Downlinks', 'Mismatches', 'Mismatch\u2194Downlink', 'Optics', 'combined_fec', 'cannot_decode_sp']
    ...
    # ── 8f. Highlight reciprocal mismatch pairs / N-way cycles ─────────────────
    log("  · Highlighting reciprocal mismatch pairs/cycles")
    highlight_mismatch_pairs(wb, log)

    # ── 8g. Build Mismatch↔Downlink cross-reference tab ────────────────────────
    log("  · Building Mismatch↔Downlink tab")
    build_downlink_mismatch_tab(wb, log)

    # Re-sort tabs now Mismatch↔Downlink may have been added
...
# ── Mismatch pair/cycle detection / highlight ─────────────────────────────────
...
def main():
    ...
    # ── Step 1: pick Cutsheet ────────────────────────────────────────────────
    cutsheet_path = pick_file("Select Cutsheet (Installation Sheet)")
    if not cutsheet_path:
        messagebox.showerror("Cancelled", "No cutsheet selected.")
        sys.exit(1)
    ...
    try:
        ...
        messagebox.showerror("Error loading cutsheet", str(e))
        sys.exit(1)
    ...
    for i, input_path in enumerate(input_files, 1):
        ...
        try:
            process_file(input_path, out_path, cut_df, pw.log)
        except Exception as e:
            pw.log(f"  ✗ ERROR: {e}")
