...
# Rack type mapping is driven by the authoritative Placement Groups file.
# This ensures the dashboard GPU section only shows racks whose SKU is exactly
# GPU_GB300_NVL72_R.03, and allows future differentiation (T0, T1, Aux, IPR, etc.).
PLACEMENT_GROUPS_FILE = Path(__file__).parent.parent / "HSG17_Placement_Groups.txt"

def _load_rack_to_sku() -> dict[str, str]:
    rack_to_sku: dict[str, str] = {}
    if not PLACEMENT_GROUPS_FILE.exists():
        # Fallback: keep a minimal set so the app doesn't break if the file is missing
        print(f"[hsg17_models] Warning: {PLACEMENT_GROUPS_FILE} not found. Using limited fallback for GPU detection.")
        return {"0502": "GPU_GB300_NVL72_R.03"}  # minimal example

    try:
        with open(PLACEMENT_GROUPS_FILE, "r", encoding="utf-8", errors="ignore") as f:
            for line in f:
                parts = [p.strip() for p in line.strip().split("\t") if p.strip()]
                if len(parts) >= 3:
                    sku = parts[1]
                    rack = parts[2]
                    # Handle normal 4-digit racks
                    if re.fullmatch(r"\d{4}", rack):
                        rack_to_sku[rack] = sku
                    # Handle multi-rack aux lines like "5920\n6002"
                    elif "\n" in rack:
                        for r in rack.split():
                            if re.fullmatch(r"\d{4}", r):
                                rack_to_sku[r] = sku
    except Exception as e:
        print(f"[hsg17_models] Failed to parse Placement Groups file: {e}")

    return rack_to_sku

RACK_TO_SKU: dict[str, str] = _load_rack_to_sku()

# GPU racks are those whose SKU in the Placement Groups file is exactly GPU_GB300_NVL72_R.03
GPU_RACKS: set = {r for r, sku in RACK_TO_SKU.items() if "GPU_GB300_NVL72_R.03" in sku}

# Human-friendly type mapping (for future dashboard improvements, logging, etc.)
SKU_TO_TYPE: dict[str, str] = {
    "GPU_GB300_NVL72_R.03": "GPU",
    "net.ad_spc4_planar_qfab_t0_1.01": "Qfab T0",
    "net.ad_spc4_planar_qfab_t1_1.01": "Qfab T1",
    "net.ad_spc4_planar_qfab_ipr_1.01": "IPR",
    "aux.02 aux.01": "Aux Rack",
}

def get_rack_type(rack: Optional[str]) -> str:
    """Return human-friendly type for a rack (GPU, Qfab T0, Aux Rack, etc.)."""
    if not rack:
        return "Unknown"
    r = str(rack).strip()
    if r.upper().startswith("RACK "):
        ...
        if len(parts) > 1:
            r = parts[1]
    r = r.zfill(4)
    sku = RACK_TO_SKU.get(r) or RACK_TO_SKU.get(r.lstrip("0") or "0")
    if not sku:
        return "Unknown"
    return SKU_TO_TYPE.get(sku, sku)  # fallback to raw sku if unknown

def is_gpu_rack(rack: Optional[str]) -> bool:
    """Return True only if the rack's SKU in the Placement Groups file is GPU_GB300_NVL72_R.03."""
    if not rack:
        return False
    r = str(rack).strip()
    if r.upper().startswith("RACK "):
